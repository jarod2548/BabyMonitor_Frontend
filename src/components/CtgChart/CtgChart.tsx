    ...
    <div className="ctg-chart">
      ...
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="timestamp" minTickGap={24} tickFormatter={formatTimestamp} />
          <YAxis yAxisId="toco" width={50} domain={[90, 190]} label={{ value: "TOCO", angle: -90, position: "insideLeft" }} />
          <YAxis yAxisId="toco" width={40} domain={[90, 190]} label={{ value: "TOCO", angle: -90, position: "insideLeft" }} />
          <Tooltip labelFormatter={(value) => formatTimestamp(String(value))} />
          <Legend />
